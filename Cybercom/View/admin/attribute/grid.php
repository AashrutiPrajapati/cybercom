    ...
    <?php else :    
        foreach ($data->getData() as $val) {?>
         <tr>
                ...
                <td><a onclick="object.setUrl('<?php echo $this->getUrl('edit', null, ['id' => $val->attributeId]) ?>').load()" class="btn btn-info btn-sm">Edit</a>
                    <a onclick="object.setUrl('<?php echo $this->getUrl('delete', null, ['id' => $val->attributeId]) ?>').load()" class="btn btn-danger btn-sm">Delete</a></td>
            </tr>
        <?php } endif; ?>
        </table> 
    </div>   
